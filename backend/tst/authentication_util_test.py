...
from ..src import authentication_util, model


class TestAuthenticationUtil(unittest.TestCase):

    def setUp(self):
        self.valid_user = model.User(id=1, email="test@example.com", fake_hashed_password="hashed_password")
        self.valid_password = "hashed_password"
        self.invalid_password = "wrong_password"
        self.jwt_secret = "jwtsecret"
        self.patcher = patch("backend.src.authentication_util.search_for_user_in_db")
    ...
    @patch("..src.authentication_util.search_for_user_in_db", return_value=None)
    ...
    @patch("..src.authentication_util.search_for_user_in_db", return_value=model.User(id=1, email="test@example.com", fake_hashed_password="hashed_password"))
    ...
    @patch("backend.src.authentication_util.jwt.decode")
    def test_get_current_user_success(self, mock_jwt_decode):
        mock_jwt_decode.return_value = {"email": self.valid_user.email}
        self.mock_search_for_user.return_value = self.valid_user
        result = authentication_util.get_current_user(token="valid_token")
        self.assertEqual(result, self.valid_user.id)

    @patch("backend.src.authentication_util.jwt.decode", side_effect=JWTError)
